...
window.ConfigurationsView = countlyView.extend({
    ...
    getLabelName: function(id, value) {
        ...
        if (typeof this.predefinedLabels[id] !== "undefined") {
            return { dataLocalize: this.predefinedLabels[id],
                text: jQuery.i18n.map[this.predefinedLabels[id]] };
        }
        else if (jQuery.i18n.map["configs." + id]) {
            return { dataLocalize: 'configs." + id + "',
                text: jQuery.i18n.map["configs." + id] };
        }
        else if (jQuery.i18n.map["configs." + id.replace(".", "-")]) {
            return { dataLocalize: 'configs." + id.replace(".", "-") + "',
                text: jQuery.i18n.map["configs." + id.replace(".", "-")] };
        }
        else if (jQuery.i18n.map[id]) {
            return { dataLocalize: id,
                text: jQuery.i18n.map[id] };
        }
        else if (jQuery.i18n.map[id.replace(".", "-")]) {
            return { dataLocalize: '" + id.replace(".", "-") + "',
                text: jQuery.i18n.map[id.replace(".", "-")] };
        }
        else {
            return { text: value };
        }
...
if (countlyGlobal["member"].global_admin) {
    var showInAppManagment = {"api": {"safe": true,
...
app.addPageScript("/manage/plugins", function() {
    ...
    $(document).on("click", ".btn-plugin-enabler", function() {
        ...
        var msg = { title: jQuery.i18n.map["plugins.processing"],
            message: jQuery.i18n.map["plugins.wait"],
            info: jQuery.i18n.map["plugins.hold-on"],
            sticky: true };
        ...
        }, [jQuery.i18n.map["common.no-dont-continue"], jQuery.i18n.map["plugins.yes-i-want-to-apply-changes"]], { title: jQuery.i18n.map["plugins-apply-changes-to-plugins"],
            image: "apply-changes-to-plugins" });
    });
});
