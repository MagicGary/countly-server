...
(function(plugin) {
    var logs = {api: "../../../log/countly-api.log",
        dashboard: "../../../log/countly-dashboard.log"};
                ...
                if (params.qstring.download) {
                    if (bytes === 0) {
                        fs.readFile(dir + "/" + logs[params.qstring.log], 'utf8', function(err, data) {
                            ...
                            common.returnRaw(params, 200, data, {'Content-Type': 'plain/text; charset=utf-8',
                                'Content-disposition': 'attachment; filename=countly-' + params.qstring.log + '.log'});
                        });
                    }
                    else {
                        readFromEnd(dir + "/" + logs[params.qstring.log], bytes)
                            .then(function(data) {
                                ...
                                if (!params.res.finished) {
                                    common.returnRaw(params, 200, "", {'Content-Type': 'plain/text; charset=utf-8',
                                        'Content-disposition': 'attachment; filename=countly-' + params.qstring.log + '.log'});
                                }
                            });
                    }
        ...
        validate(params, function(params) {
            if (params.qstring.log && logs[params.qstring.log]) {
                plugins.dispatch("/systemlogs", {params: params,
                    action: "errologs_clear",
                    data: {log: params.qstring.log}});
