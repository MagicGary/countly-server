var alertDefine = {
    metric: {
        ...
        condition: [
            { value: 'increased by at least',
                name: 'increased by at least' },
            { value: 'decreased by at least',
                name: 'decreased by at least' },
        ]
    },
    event: {
        target: [],
    ...
    crash: {
        target: [
            { value: 'Total crashes',
                name: 'Total crashes' },
            { value: 'New crash occurence',
                name: 'New crash occurence' },
            { value: 'None fatal crash per session',
                name: 'None fatal crash per session' },
            { value: 'Fatal crash per session',
                name: 'Fatal crash per session' },
        ],
        condition: [
            { value: 'increased by at least',
                name: 'increased by at least' },
...
window.AlertsView = countlyView.extend({
    ...
    renderTable: function() {
        ...
        $(".delete-alert").off("click").on("click", function(e) {
            ...
            }, [jQuery.i18n.map["common.no-dont-delete"], jQuery.i18n.map["alert.yes-delete-alert"]], {title: jQuery.i18n.map["alert.delete-confirm-title"],
                image: "delete-alert"});
        });
        ...
        loadData: function(data) {
            ...
            case 'crash':
                ...
                for (var index in data.selectedApps) {
                    ...
                    countlyGlobal.apps[appId] && appSelected.push({ value: appId,
                        name: countlyGlobal.apps[appId].name });
                }
