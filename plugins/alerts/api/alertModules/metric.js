 				...
 				html = yield utils.getEmailTemplate({
                    ...
                    apps: result.map((data)=>{
                        ...
                        if (data.lastDateValue !== null) {
                            item['data'].push({key: 'Yesterday\'s Value',
                                value: data.lastDateValue});
                        }
                        return item;
                    })
                });
        ...
        return bluebird.coroutine(function *() {
            ...
            for (let i = 0; i < alertConfigs.selectedApps.length; i++) {
                ...
                if (alertConfigs.alertDataSubType === 'Average session duration') {
                    ...
                    const result = utils.compareValues(alertConfigs, {todayValue,
                        lastDateValue}, null, i);
...
function getUserAndSessionData(db, app_id, period) {
    return new bluebird(function(resolve, reject) {
        return fetch.fetchTimeObj('users', {app_id,
            qstring: {period}}, false, function(data) {
            return resolve(data);
        });
    });
}
...
function getViewData(app_id) {
    return new bluebird(function(resolve, reject) {
        fetch.getTimeObjForEvents("app_viewdata" + app_id, {app_id,
            qstring: {}}, {unique: "u",
