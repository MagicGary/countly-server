...
function check_ext(file) {
    ...
    CountlyHelpers.alert(jQuery.i18n.map["plugin-upload.badformat"], "popStyleGreen", {title: jQuery.i18n.map["common.error"],
        image: "token-warning"});
    return false;
}
    ...
    app.addPageScript("/manage/plugins", function() {
        $(document).ready(function() { //creates upload form
            $.when($.get(countlyGlobal["path"] + '/plugin-upload/templates/drawer.html', function(src) {
                ...
                myDropzone = new Dropzone("#plugin-upload-drop", {url: '/',
                    autoQueue: false,
                ...
                $("#upload-new-plugin").click(function() {
                    ...
                    var msg = {title: jQuery.i18n.map["plugin-upload.processing"],
                        message: jQuery.i18n.map["plugin-upload.saving-data"],
                        sticky: true};
                    ...
                    $('#upload-plugin-form').ajaxSubmit({
                        beforeSubmit: function(formData, jqForm, options) {
                            ...
                            if (myDropzone && myDropzone.files && myDropzone.files.length > 0) {
                                formData.push({ name: 'new_plugin_input',
                        ...
                        success: function(result) {
                            ...
                            if (aa.length === 2 && aa[0] === 'Success') {
                                ...
                                $.when(app.pluginsView.refresh(true)).then(
                                    function() {
                                        var msg = {title: jQuery.i18n.map["plugin-upload.success"],
                                            message: jQuery.i18n.map["plugin-upload.success"],
                                            clearAll: true,
                                            sticky: true};
                                        CountlyHelpers.notify(msg);
                                        //highlight
                                ...
                                var msg = {title: jQuery.i18n.map["plugin-upload.error"],
                                    message: jQuery.i18n.map['plugin-upload.' + result],
                                    sticky: true,
                                    clearAll: true,
                                    type: "error"};
                                CountlyHelpers.notify(msg);
