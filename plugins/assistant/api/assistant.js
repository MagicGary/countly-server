...
(function(assistant) {
    ...
    assistant.createNotification = function(db, data, pluginName, type, subtype, i18n, appId, notificationVersion, targetUserApiKey, batchInfoHolder, callback) {
        try {
            ...
            log.e('assistant.createNotification error:[%j]', { message: ex.message,
                stack: ex.stack });
            if (callback !== null) {
                callback();
            }
    ...
    insertNotificationBulk = function(db, notifData, callback) {
        try {
            ...
            log.i('insertNotificationBulk error:[%j]', { message: ex.message,
                stack: ex.stack });
        }
    };
    ...
    assistant.increaseNotificationShowAmount = function(db, anc, appID, batchInfoHolder, callback) {
        ...
        if (batchInfoHolder === null) {
            doNotificationShowAmountUpdateBulk(db, [{appID: appID,
                updateQuery: updateQuery}], function(err, res) {
                if (callback !== null) {
                    callback();
                }
            });
            ...
            batchInfoHolder.push({appID: appID,
    ...
    assistant.preparePluginSpecificFields = function(assistantGlobalCommon, appData, PLUGIN_NAME) {
        ...
        try {
            ...
            log.w('Assistant plugin got exception while trying to set timezone [%j]', { message: ex.message,
                stack: ex.stack });
        }
