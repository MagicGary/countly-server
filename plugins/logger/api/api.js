...
(function(plugin) {
        ...
        if (params.qstring.method === 'collection_info') {
            validate(params, function(params) {
                common.db.collection('logs' + params.app_id).stats(function(err, stats) {
                    ...
                    common.returnOutput(params, stats && {capped: stats.capped,
                        max: stats.max} || {});
				  });
            });
            return true;
    ...
    plugins.register("/i/apps/create", function(ob) {
        var params = ob.params;
        var appId = ob.appId;
        common.db.command({"convertToCapped": 'logs' + appId,
            size: 10000000,
            max: 1000}, function(err, data) {
            if (err) {
                common.db.createCollection('logs' + appId, {capped: true,
                    size: 10000000,
                    max: 1000}, function(err, data) {});
            }
        });
