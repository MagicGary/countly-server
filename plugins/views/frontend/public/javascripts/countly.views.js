        ...
        if (!isRefresh) {
                ...
                if ($(event.target).hasClass("active")) {
                    ...
                    countlyTokenManager.createToken("View heatmap", "/o/actions", false, countlyCommon.ACTIVE_APP_ID, 1800, function(err, token) {
                        ...
                        if (followLink && self.token) {
                            ...
                            newWindow.name = "cly:" + JSON.stringify({"token": self.token,
                                "purpose": "heatmap",
            ...
            $('.widget-content .cly-button-menu .item').click(function(event) {
                ...
                if (self.token !== false) {
                    ...
                    window.open(url + path, "cly:" + JSON.stringify({"token": self.token,
                        "purpose": "heatmap",
                        period: countlyCommon.getPeriodForAjax(),
                        showHeatMap: true}));
                }
    ...
    renderCommon: function(isRefresh) {
        ...
        if (!isRefresh) {
            ...
            this.dtable = $('.d-table').dataTable($.extend({}, $.fn.dataTable.defaults, {
                ...
                "aoColumns": [
                    { "mData": "vc",
                        sType: "view-frequency",
                        "sTitle": jQuery.i18n.map["views.view-frequency"] },
                    { "mData": "t",
                        sType: "formatted-num",
                        "mRender": function(d) {
                            return countlyCommon.formatNumber(d);
