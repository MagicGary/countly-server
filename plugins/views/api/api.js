...
(function(plugin) {
    ...
    plugins.register("/o", function(ob) {
        ...
        if (params.qstring.method === "views") {
            validateUserForDataReadAPI(params, function() {
                fetch.getTimeObjForEvents("app_viewdata" + params.app_id, params, {unique: "u",
                    levels: {daily: ["u", "t", "s", "b", "e", "d", "n"],
                        monthly: ["u", "t", "s", "b", "e", "d", "n"]}}, function(data) {
                ...
                common.db.collection("app_viewdata" + params.app_id).findOne({'_id': "meta"}, function(err, res1) {
                    ...
                    common.db.collection("app_viewdata" + params.app_id).findOne({'_id': "meta_v2"}, function(err, res2) {
                        ...
                        if (common.drillDb) {
                            ...
                            common.drillDb.collection(collectionName).findOne({"_id": "meta_v2"}, {_id: 0,
                                "sg.domain": 1}, function(err, meta) {
                                if (meta && meta.sg && meta.sg.domain.values) {
                                    res.domains = Object.keys(meta.sg.domain.values);
                                }
                                common.drillDb.collection(collectionName).findOne({"_id": "meta"}, {_id: 0,
                                    "sg.domain": 1}, function(err, meta2) {
                                    ...
                                    common.drillDb.collection(collectionName).findOne({"_id": "meta_" + eventHash}, {_id: 0,
                                        "sg.domain": 1}, function(err, meta) {
                                        if (meta && meta.sg && meta.sg.domain.values) {
    ...
    function getHeatmap(params) {
        ...
        common.drillDb.collection(collectionName).findOne({"_id": "meta_v2"}, {_id: 0,
            ...
            common.drillDb.collection(collectionName).findOne({"_id": "meta"}, {_id: 0,
                ...
                common.drillDb.collection(collectionMeta).findOne({"_id": "meta_" + eventHash}, {_id: 0,
                    "sg.domain": 1}, function(err, meta_event) {
                                ...
                                authorize.save({db: common.db,
                                    ...
                                    callback: function(err, token) {
                                        params.token_headers = {"countly-token": token,
                                            ...
                                            "Access-Control-Expose-Headers": "countly-token"};
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (dbAppUser && dbAppUser.vc) {
                ...
                if (user && user.vc) {
                    ...
                    var update = {'$inc': updateUsersZero,
                        '$set': {}};
                    ...
                    if (user.lv) {
                        if (ob.end_session || user.lvt && params.time.timestamp - user.lvt > 300) {
                            var segmentation = {name: user.lv.replace(/^\$/, "").replace(/\./g, "&#46;"),
                                exit: 1};
                            if (user.vc === 1) {
                                segmentation.bounce = 1;
                            }
                            recordMetrics(params, {key: "[CLY]_view",
                                segmentation: segmentation}, user);
                        }
                    }
                    ...
                    if (currEvent.key === "[CLY]_view") {
                        if (currEvent.segmentation && currEvent.segmentation.name) {
                            ...
                            if (currEvent.segmentation.visit) {
                                ...
                                if (currEvent.dur) {
                                    plugins.dispatch("/view/duration", {params: params,
                                        duration: currEvent.dur});
                                }
                            }
                        }
                        return false;
    ...
    function processView(params, currEvent) {
        ...
        if (currEvent.segmentation.visit) {
            ...
            common.db.collection('app_views' + params.app_id).findAndModify({'_id': params.app_user_id }, {}, {$max: lastView}, {upsert: true,
                new: false}, function(err, view) {
    ...
    function recordMetrics(params, currEvent, user, view) {
        ...
        common.db.collection("app_viewdata" + params.app_id).findOne({'_id': tmpZeroId}, {meta_v2: 1}, function(err, res) {
            ...
            if (Object.keys(tmpTimeObjMonth).length) {
                var update = {$set: {m: dateIds.month,
                    a: params.app_id + ""}};
                if (Object.keys(tmpTimeObjMonth).length) {
    ...
    plugins.register("/dashboard/data", function(ob) {
        return new Promise((resolve, reject) => {
            ...
            if (data.widget_type === "views") {
                fetch.getTimeObjForEvents("app_viewdata" + params.app_id, params, {unique: "u",
                    levels: {daily: ["u", "t", "s", "b", "e", "d", "n"],
                        monthly: ["u", "t", "s", "b", "e", "d", "n"]}}, function(res) {
