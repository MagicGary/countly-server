window.DataMigrationView = countlyView.extend({
    ...
    check_ext: function(file) {
        ...
        CountlyHelpers.alert(jQuery.i18n.map["data-migration.badformat"], "popStyleGreen", {title: jQuery.i18n.map["common.error"],
            image: "token-warning"});
        return false;
    },
    ...
    renderCommon: function(isRefresh) {
            ...
            $("#show_data_import_form").on("click", function() {
                ...
                $(".cly-drawer").find(".close").off("click").on("click", function() {
                    ...
                    if ($('#data_migration_generated_token').hasClass('newTokenIsGenerated')) {
                        ...
                        }, [jQuery.i18n.map["data-migration.cancel"], jQuery.i18n.map['data-migration.continue-and-close']], {title: jQuery.i18n.map["data-migration.close-confirm-title"],
                            image: "token-warning"});
                    }
                    else {
                        self.reset_import_tab();
        ...
        $('#data_migration_exports').on('click', '.resend_export', function() {
            ...
            for (var i = 0; i < myapps.length; i++) {
                selected_apps.push({value: myapps[i],
                    name: countlyGlobal.apps[myapps[i]].name});
            }
                ...
                if (result && result['result'] === 'success') {
                    if (result.data && self.explanations[result.data]) {
                        var msg = {title: jQuery.i18n.map["common.success"],
                            message: self.get_translation(result.data),
                            info: "",
                            sticky: false,
                            clearAll: true,
                            type: "info"};
        ...
        $('#data_migration_exports').on('click', '.delete_export', function() {
            ...
            }, [jQuery.i18n.map["common.no-dont-delete"], jQuery.i18n.map["data-migration.yes-delete-export"]], {title: jQuery.i18n.map["data-migration.delete-export"] + "?",
                image: "delete-exports"});
        });
        ...
        $("#export_data_button").click(function() {
            ...
            $('#export_data_form').ajaxSubmit({
                beforeSubmit: function(formData, jqForm, options) {
                    ...
                    if (download_me) {
                        formData.push({ name: 'only_export',
                            value: '1' });
                    }
                    else {
                        formData.push({ name: 'only_export',
                    ...
                    if (isChecked) {
                        formData.push({name: 'aditional_files',
                            value: '1'});
                    }
                    isChecked = $("#migration_redirect_traffic").hasClass("fa-check-square");//is now checked
                    if (isChecked) {
                        formData.push({name: 'redirect_traffic',
                            value: '1'});
                    }

                    formData.push({ name: 'apps',
                        value: applist.join() });
                },
                success: function(result) {
                    ...
                    var msg = {title: jQuery.i18n.map["common.success"],
                        message: jQuery.i18n.map["data-migration.export-started"],
                        sticky: false,
                        clearAll: true,
                        type: "info"};
                    CountlyHelpers.notify(msg);
    ...
    create_import_tab: function() {
        ...
        //file oploader
        myDropzone = new Dropzone("#data-migration-import-via-file", {url: '/',
            autoQueue: false,
        ...
        $("#create_new_token").click(function() {
            ...
            var msg = {title: jQuery.i18n.map["data-migration.please-wait"],
                message: jQuery.i18n.map["data-migration.creating-new-token"],
                sticky: false};
            CountlyHelpers.notify(msg);
            ...
            $.when(countlyDataMigration.createToken(function(result) {
                ...
                if (result && result['result'] === 'success') {
                    ...
                    var msg = {title: jQuery.i18n.map["data-migration.complete"],
                        message: jQuery.i18n.map["data-migration.new-token-created"],
                        sticky: false,
                        clearAll: true};
                    CountlyHelpers.notify(msg);
                }
                    ...
                    resp = self.get_response_text(result.data.xhr, result.data.status, result.data.error);
                    resp = self.get_translation(resp);
                    var msg = {title: jQuery.i18n.map["data-migration.error"],
                        message: jQuery.i18n.map["data-migration.unable-create-token"],
                        info: resp,
                        sticky: true,
                        clearAll: true,
                        type: "error"};
        ...
        $("#import_data_button").click(function() {
            if (!$(this).hasClass("disabled")) {
                ...
                CountlyHelpers.notify({title: jQuery.i18n.map["common.success"],
                    message: "Uploading file....",
                    sticky: true});
                $('#import_data_form').ajaxSubmit({
                    beforeSubmit: function(formData, jqForm, options) {
                        if (myDropzone && myDropzone.files && myDropzone.files.length > 0) {
                            formData.push({ name: 'import_file',
                                value: myDropzone.files[myDropzone.files.length - 1] });
                        }
                    },
                    success: function(result) {
                        overlay.hide();
                        if (result['result']) {
                            if (result['result'].substr(0, 26) === 'Importing process started.') {
                                var msg = {title: jQuery.i18n.map["common.success"],
                                    message: jQuery.i18n.map["data-migration.import-started"],
                                    sticky: false,
                                    clearAll: true};
                    ...
                    error: function(xhr, status, error) {
                        ...
                        var msg = {title: jQuery.i18n.map["common.error"],
                            message: jQuery.i18n.map["systemlogs.action.import_failed"],
                            sticky: false,
                            clearAll: true,
                            type: "error"};
                        CountlyHelpers.notify(msg);
...
if (countlyGlobal["member"].global_admin) {
    ...
    $(document).ready(function() {
        ...
        if (curapp && countlyGlobal['apps'][curapp] && countlyGlobal['apps'][curapp]['redirect_url'] && countlyGlobal['apps'][curapp]['redirect_url'] !== "") {
            ...
            var msg = {title: jQuery.i18n.map["data-migration.app-redirected"].replace('{app_name}', countlyGlobal['apps'][curapp]['name']),
                message: mm,
    ...
    app.addAppSwitchCallback(function(appId) {
        if (appId && countlyGlobal['apps'][appId] && countlyGlobal['apps'][appId]['redirect_url'] && countlyGlobal['apps'][appId]['redirect_url'] !== "") {
            ...
            var msg = {title: jQuery.i18n.map["data-migration.app-redirected"].replace('{app_name}', countlyGlobal['apps'][appId]['name']),
                message: mm,
                info: jQuery.i18n.map["data-migration.app-redirected-remove"],
                sticky: true,
                clearAll: true,
