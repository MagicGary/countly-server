...
module.exports = function(my_db) {
            ...
            if ((status === 'failed' || status === 'finished')) {
                db.collection("data_migrations").findOne({_id: my_exportid}, function(err, res) {
                    if (err) { }
                        ...
                        if (res) {
                            plugins.dispatch("/systemlogs", {params: {req: JSON.parse(res.myreq)},
                                user: {_id: res.userid,
                                    email: res.email},
                                action: "export_" + status,
                                data: {app_ids: res.apps,
                                    status: status,
                                    message: reason}});
                        }
                    }
        ...
        return new Promise(function(resolve, reject) {
            ...
            db.collection("apps").findOne({_id: db.ObjectID(appid)}, function(err, res) {
                if (err || !res) {
                    ...
                    //events sctipts
                    generate_events_scripts({appid: appid,
                        my_folder: my_folder,
                        dbstr: dbstr,
                        dbstr_drill: dbstr_drill}, db)
                        .then(
                            function(result) {
                                ...
                                return generate_credentials_scripts({appid: appid,
                                    my_folder: my_folder,
                                    dbstr: dbstr,
                                    dbstr_drill: dbstr_drill});
                            })
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (!fs.existsSync(folder)) {
                ...
                for (var i = 0; i < myfiles.length; i++) {
                    objlist.push({imagepath: path.resolve(__dirname, './../../../frontend/express/public/appimages/' + myfiles[i]),
                        source: folder + '/' + myfiles[i],
                        id: myfiles[i]});
                }
            ...
            if (!fs.existsSync(folder)) {
                ...
                for (var i = 0; i < myapps.length; i++) {
                    ...
                    for (var j = 0; j < myfiles.length; j++) {
                        objlist.push({imagepath: path.resolve(__dirname, './../../crash_symbolication/crashsymbols/' + myapps[i] + '/' + myfiles[j]),
                            source: folder + '/' + myapps[i] + '/' + myfiles[j],
                            id: myapps[i] + '.' + myfiles[j]});
                    }
                }
        ...
        return new Promise(function(resolve, reject) {
            ...
            db.collection("app_crashsymbols" + data.appid).find().toArray(function(err, res) {
                if (err) {
                    ...
                    var symb_files = [];
                    for (var i = 0; i < res.length; i++) {
                        symb_files.push({folder: data.aditional_files,
                            symbolid: res[i]._id,
                            appid: data.appid});
                    }
    ...
    var report_import = function(params, message, status, my_exportid) {
        ...
        var moredata = {"app_ids": imported_ids,
            "app_names": imported_apps,
            "exportid": my_exportid,
            reason: message};
        if (params && params.qstring && params.qstring.exportid) {
            ...
            var r = request.post({url: 'http://' + moredata.serverip + '/i/datamigration/report_import?token=' + moredata.token + "&exportid=" + my_exportid + "&status=" + status + "&message=" + message,
                agentOptions: {rejectUnauthorized: false}}, function(err, res, body) {
                if (err) {
                    plugins.dispatch("/systemlogs", {params: params,
                        action: "import_" + status + "_response_failed",
                        data: moredata});
                }
                else {
                    if (res.statusCode >= 400 && res.statusCode < 500) {
                        ...
                        plugins.dispatch("/systemlogs", {params: params,
                            action: "import_" + status + "_response_ok",
                            data: moredata});
                    }
                }
        ...
        return new Promise(function(resolve, reject) {
            ...
            check_ids(apps).then(
                ...
                function(result) {
                    ...
                    update_progress(exportid, "exporting", "progress", 0, "", true, {created: created,
                        stopped: false,
                        only_export: params.qstring.only_export,
                        server_address: params.qstring.server_address,
                        server_token: params.qstring.server_token,
                        redirect_traffic: params.qstring.redirect_traffic,
                        aditional_files: params.qstring.aditional_files,
                        apps: apps,
                        app_names: app_names,
                        userid: params.member._id,
