...
function apply_redirect_to_apps(apps, params, my_redirect_url, userid, email) {
    return new Promise(function(resolve, reject) {
        if (!my_redirect_url || my_redirect_url === "") {
            ...
            common.db.collection("apps").update({_id: {$in: object_array}}, {$set: {redirect_url: my_redirect_url}}, {upsert: true,
                multi: true}, function(err, res) {
                if (err) {
                    resolve(err);
                }
                else {
                    plugins.dispatch("/systemlogs", {params: {req: JSON.parse(params)},
                        user: {_id: userid,
                            email: email},
                        action: "app_redirected",
                        data: {app_id: apps,
...
(function(plugin) {
    ...
    plugins.register("/i/datamigration/report_import", function(ob) {
        ...
        if (params.qstring) {
            ...
            common.db.collection("data_migrations").findOne({_id: params.qstring.exportid,
                server_token: params.qstring.token}, function(err, res) {
                if (err) {
    ...
    plugins.register("/o/datamigration/createimporttoken", function(ob) {
        ...
        validate(params, function() {
            ...
            authorize.save({endpoint: ['/i/datamigration/import'],
                db: common.db,
                ttl: ttl,
                multi: multi,
    ...
    plugins.register("/i/datamigration/sendexport", function(ob) {
        ...
        validate(params, function() {
            ...
            if (params.qstring.exportid) {
                ...
                update_progress(params.qstring.exportid, "packing", "progress", 100, "", true, {stopped: false,
                    only_export: false,
