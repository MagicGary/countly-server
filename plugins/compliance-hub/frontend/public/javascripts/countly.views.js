                ...
                "fnServerParams": function(aoData) {
                    if (self.history_filter) {
                        aoData.push({ "name": "filter",
                            "value": JSON.stringify(self.history_filter) });
                    }
                    aoData.push({ "name": "period",
                        "value": countlyCommon.getPeriodForAjax() });
                },
                    ...
                    if (el.hasClass("view-history")) {
                        ...
                        countlyAppUsers.exportUser(JSON.stringify({uid: id}), function(error, export_id, task_id) {
                            if (error) {
                                ...
                                CountlyHelpers.notify({type: "ok",
                                    title: jQuery.i18n.map["common.success"],
                                    message: jQuery.i18n.map["app-users.export-started"],
                                    sticky: false,
                                    clearAll: false});
                                self.dtableusers.fnDraw(false);
                            }
                        ...
                        countlyAppUsers.deleteExport(id, function(error, res) {
                            if (error) {
                                ...
                                CountlyHelpers.notify({type: "ok",
                                    title: jQuery.i18n.map["common.success"],
                                    message: jQuery.i18n.map["app-users.export-deleted"],
                                    sticky: false,
                                    clearAll: true});
                                self.dtableusers.fnDraw(false);
                            }
                        ...
                        CountlyHelpers.confirm(jQuery.i18n.map["app-users.delete-userdata-confirm"], "popStyleGreen", function(result) {
                            ...
                            countlyAppUsers.deleteUserdata(JSON.stringify({uid: id}), function(error, res) {
                                if (error) {
                                    ...
                                    CountlyHelpers.notify({type: "ok",
                                        title: jQuery.i18n.map["common.success"],
                                        message: jQuery.i18n.map["app-users.userdata-deleted"],
                                        sticky: false,
                                        clearAll: true});
                                    self.dtableusers.fnDraw(false);
                                }
                            });
                        }, [jQuery.i18n.map["app-users.no-dont-purge"], jQuery.i18n.map["app-users.yes-purge-data"]], {title: jQuery.i18n.map["app-users.purge-confirm-title"],
                            image: "purge-user-data"});
                    }
                }
            });
