...
(function(plugin) {
            ...
            common.db.collection("app_crashusers" + appId).find({uid: oldUid}).toArray(function(err, res) {
                if (res && res.length) {
                    ...
                    for (let i = 0; i < res.length; i++) {
                        ...
                        bulk.find({uid: newUid,
                            group: group}).upsert().updateOne(updates);
                        bulk.find({uid: oldUid,
                            group: group}).remove();
                    }
                    bulk.execute();
                }
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (!params.qstring.crash && params.qstring.metrics && params.qstring.metrics._app_version) {
                    ...
                    common.db.collection('app_crashgroups' + params.app_id).findOne({'_id': hash }, function(err, crash) {
                        if (crash && crash.is_resolved && crash.resolved_version) {
                            if (common.versionCompare(latest_version, crash.resolved_version.replace(/\./g, ":")) > 0) {
                                ...
                                //update crash stats
                                common.db.collection('app_crashusers' + params.app_id).remove({"group": hash,
                                    uid: uid}, function() {});
                                common.db.collection('app_crashgroups' + params.app_id).update({'_id': hash,
                                    users: {$gt: 0} }, {$inc: {users: -1}}, function(err, res) {});
                                ...
                                common.db.collection('app_crashusers' + params.app_id).findAndModify({"group": 0,
                                    uid: uid}, {}, {$inc: mod}, {upsert: true,
                                    new: true}, function(err, res) {
                                    res = res && res.ok ? res.value : null;
                                    if (res && res.crashes <= 0) {
                                        common.db.collection('app_crashusers' + params.app_id).remove({"group": 0,
                                            uid: uid}, function() {});
                                    }
                                    done(null, true);
                                });
                ...
                if (dbAppUser && dbAppUser.uid && (!dbAppUser.av || common.versionCompare(latest_version, dbAppUser.av) > 0)) {
                    common.db.collection('app_crashusers' + params.app_id).find({uid: dbAppUser.uid}, {group: 1,
                        _id: 0}).toArray(function(err, res) {
                        if (res && res.length) {
                            var crashes = [];
                            ...
                            if (crashes.length) {
                                async.map(crashes, function(crash, done) {
                                    ...
                                    if (shouldRecalculate) {
                                        common.db.collection('app_crashusers' + params.app_id).count({"group": 0,
                                            crashes: { $gt: 0 }}, function(err, userCount) {
                                            common.db.collection('app_crashusers' + params.app_id).count({"group": 0,
                                                crashes: { $gt: 0 },
                                                fatal: { $gt: 0 }}, function(err, fatalCount) {
                                                var set = {};
                                                set.users = userCount;
                ...
                if (error !== "") {
                    ...
                    function checkUser(dbAppUser, tries) {
                        if (!dbAppUser || !dbAppUser.uid) {
                            ...
                            common.db.collection('app_crashusers' + params.app_id).findAndModify({group: hash,
                                'uid': report.uid}, {}, {$set: set,
                                $inc: {reports: 1}}, {upsert: true,
                                ...
                                common.db.collection('app_crashes' + params.app_id).insert(report, function(err, res) {
                                    if (res && res.insertedIds && res.insertedIds[0]) {
                                        ...
                                        plugins.dispatch("/plugins/drill", {params: params,
                                            dbAppUser: dbAppUser,
                                            events: events});
                                        ...
                                        function processCrash(userAll, lastTs) {
                                            ...
                                            common.db.collection('app_crashgroups' + params.app_id).findAndModify({'_id': hash }, {}, update, {upsert: true,
                                                new: true}, function(err, crashGroup) {
                                        ...
                                        common.db.collection('app_crashgroups' + params.app_id).findOne({_id: hash}, {fields: {_id: 0,
                                            lastTs: 1}}, function(err, group) {
                                            var lastTs;
                                            if (group) {
                                                lastTs = group.lastTs;
                                            }
                                            if (!user || !user.reports) {
                                                ...
                                                common.db.collection('app_crashusers' + params.app_id).findAndModify({group: 0,
                                                    'uid': report.uid}, {}, {$set: {group: 0,
                                                    'uid': report.uid},
                                                $inc: inc}, {upsert: true,
                                                    new: true}, function(err, userAll) {
        ...
        if (params.qstring.method === 'crashes') {
            validate(params, function(params) {
                if (params.qstring.group) {
                    if (params.qstring.userlist) {
                        common.db.collection('app_crashusers' + params.app_id).find({group: params.qstring.group}, {uid: 1,
                            _id: 0}).toArray(function(err, uids) {
                            var res = [];
                    ...
                    common.db.collection('app_crashgroups' + params.app_id).count({}, function(err, total) {
                        total--;
                        var cursor = common.db.collection('app_crashgroups' + params.app_id).find(filter, {uid: 1,
                            is_new: 1,
                            is_renewed: 1,
                            is_hidden: 1,
                            os: 1,
                        ...
                        cursor.count(function(err, count) {
                            ...
                            cursor.toArray(function(err, res) {
                                res = res || [];
                                common.returnOutput(params, {sEcho: params.qstring.sEcho,
                                    iTotalRecords: total,
            ...
            validate(params, function(params) {
                if (params.qstring.uid) {
                    ...
                    var query = {group: {$ne: 0},
                        uid: params.qstring.uid};
                    ...
                    cursor.count(function(err, total) {
                        ...
                        cursor.toArray(function(err, crashes) {
                            ...
                            if (params.qstring.fromExportAPI) {
                                common.db.collection('app_crashgroups' + params.app_id).find({_id: {$in: groupIDs}}, {name: 1,
                                    _id: 1}).toArray(function(err, groups) {
                                    if (groups) {
    ...
    plugins.register("/i/crashes", function(ob) {
                    ...
                    if (groups) {
                        ...
                        async.each(groups, function(group, done) {
                            ...
                            if (!group.is_resolved) {
                                common.db.collection('app_crashgroups' + params.qstring.app_id).update({'_id': group._id}, {"$set": {is_resolved: true,
                                    resolved_version: group.latest_version,
                                    is_renewed: false,
                                    is_new: false,
                                    is_resolving: false}}, function(err, res) {
                                    if (!inc.resolved) {
                                        inc.resolved = 0;
                                    }
                                    inc.resolved++;
                                    ...
                                    plugins.dispatch("/systemlogs", {params: params,
                                        action: "crash_resolved",
            ...
            validate(params, function(params) {
                ...
                common.db.collection('app_crashgroups' + params.qstring.app_id).find({'_id': {$in: crashes}}).toArray(function(err, groups) {
                    if (groups) {
                        var inc = {};
                        async.each(groups, function(group, done) {
                            common.db.collection('app_crashgroups' + params.qstring.app_id).update({'_id': group._id }, {"$set": {is_resolved: false,
        ...
        case 'share':
            validate(params, function(params) {
                ...
                common.db.collection('crash_share').insert({_id: id,
                    app_id: params.qstring.app_id + "",
                    crash_id: params.qstring.args.crash_id + ""}, function(err, res) {
                    ...
                    plugins.dispatch("/systemlogs", {params: params,
                        action: "crash_shared",
                        data: {app_id: params.qstring.app_id,
                            crash_id: params.qstring.args.crash_id}});
                    common.returnMessage(params, 200, 'Success');
                    return true;
                });
            });
            ...
            validate(params, function(params) {
                if (params.qstring.args.data) {
                    common.db.collection('app_crashgroups' + params.qstring.app_id).update({'_id': params.qstring.args.crash_id }, {"$set": {share: params.qstring.args.data}}, function(err, res) {
                        plugins.dispatch("/systemlogs", {params: params,
                            action: "crash_modify_share",
                            data: {app_id: params.qstring.app_id,
                                crash_id: params.qstring.args.crash_id,
                                data: params.qstring.args.data}});
                        common.returnMessage(params, 200, 'Success');
                        return true;
                    });
                }
        ...
        case 'add_comment':
            ob.validateUserForWriteAPI(function() {
                ...
                common.db.collection('app_crashgroups' + params.qstring.args.app_id).update({'_id': params.qstring.args.crash_id }, {"$push": {'comments': comment}}, function(err, res) {
                    plugins.dispatch("/systemlogs", {params: params,
                        action: "crash_added_comment",
                        data: {app_id: params.qstring.args.app_id,
                            crash_id: params.qstring.args.crash_id,
                            comment: comment}});
                    common.returnMessage(params, 200, 'Success');
                    return true;
            ...
            ob.validateUserForWriteAPI(function() {
                common.db.collection('app_crashgroups' + params.qstring.args.app_id).findOne({'_id': params.qstring.args.crash_id }, function(err, crash) {
                    ...
                    if (comment && (comment.author_id === params.member._id + "" || params.member.global_admin)) {
                        ...
                        common.db.collection('app_crashgroups' + params.qstring.args.app_id).update({'_id': params.qstring.args.crash_id,
                            "comments._id": params.qstring.args.comment_id}, {$set: {"comments.$": comment}}, function(err, res) {
                            plugins.dispatch("/systemlogs", {params: params,
                                action: "crash_edited_comment",
                                data: {app_id: params.qstring.args.app_id,
                                    crash_id: params.qstring.args.crash_id,
                                    _id: params.qstring.args.comment_id,
                ...
                common.db.collection('app_crashgroups' + params.qstring.app_id).find({'_id': {$in: crashes}}).toArray(function(err, groups) {
                    if (groups) {
                        ...
                        async.each(groups, function(group, done) {
                            ...
                            common.db.collection('crash_share').remove({'_id': id }, function(err, res) {});
                            common.db.collection('app_crashusers' + params.qstring.app_id).find({"group": group._id}, {reports: 1,
                                uid: 1,
                                _id: 0}).toArray(function(err, users) {
                                var uids = [];
                                ...
                                common.db.collection('app_crashusers' + params.qstring.app_id).update({"group": 0,
                                    uid: {$in: uids}}, {$inc: mod}, {multi: true}, function(err) {
                                    if (!inc["crashes"]) {
                                        inc["crashes"] = 0;
                                    }
                                    inc["crashes"]--;
                            ...
                            //recalculate users
                            common.db.collection('app_crashusers' + params.qstring.app_id).count({"group": 0,
                                crashes: { $gt: 0 }}, function(err, userCount) {
                                common.db.collection('app_crashusers' + params.qstring.app_id).count({"group": 0,
                                    crashes: { $gt: 0 },
                                    fatal: { $gt: 0 }}, function(err, fatalCount) {
    ...
    plugins.register("/i/apps/create", function(ob) {
        ...
        common.db.collection('app_crashusers' + appId).ensureIndex({"group": 1,
            "uid": 1}, {unique: true,
