(function(countlyCrashes, $, undefined) {
    ...
    countlyCrashes.getChartData = function(metric, name, fields) {
        ...
        if (metric === "cr-session") {
            ...
            if (fields && fields["crashes-total"] === true) {
                chartData.push({ data: [],
                    label: jQuery.i18n.map["crashes.total_overall"],
                    color: '#52A3EF' });
    ...
    countlyCrashes.getPlatformBars = function() {
        ...
        for (var i = 0; i < maxItems; i++) {
            res.push({"name": Math.round((data[i][1] / total) * 100) + "% " + data[i][0],
    ...
    countlyCrashes.getBoolBars = function(name) {
        if (_groupData[name]) {
            ...
            var ret = [];
            if (yes > 0) {
                ret.push({"name": yes.toFixed(2) + "%",
                    "percent": yes});
                ret.push({"name": no.toFixed(2) + "%",
                    "percent": no});
            }
            else {
                ret.push({"name": yes.toFixed(2) + "%",
