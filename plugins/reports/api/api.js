...
(function(plugin) {
            ...
            validate(params, function(params) {
                common.db.collection('reports').find({user: common.db.ObjectID(params.member._id)}).toArray(function(err, result) {
                    ...
                    function validateReportDispatchRequest(report, cb) {
                        plugins.dispatch("/report/verify", { params: params,
            ...
            validate(params, function(params) {
                common.db.collection('members').find({}).toArray(function(err, result) {
                    ...
                    result.forEach((member) => {
                        data.push({name: member.full_name,
                            email: member.email});
                    });
    ...
    plugins.register("/i/reports", function(ob) {
        ...
        case 'update':
            validate(function(params) {
                ...
                validationFn(params, props, function(err, authorized) {
                    ...
                    common.db.collection('reports').findOne({_id: common.db.ObjectID(id),
                        user: common.db.ObjectID(params.member._id)}, function(err, report) {
                        common.db.collection('reports').update({_id: common.db.ObjectID(id),
                            user: common.db.ObjectID(params.member._id)}, {$set: props}, function(err, app) {
                            if (err) {
                                err = err.err;
                                common.returnMessage(params, 200, err);
                            }
        ...
        case 'delete':
            validate(function(params) {
                ...
                common.db.collection('reports').findOne({'_id': common.db.ObjectID(id),
                    user: common.db.ObjectID(params.member._id)}, function(err, props) {
                    common.db.collection('reports').remove({'_id': common.db.ObjectID(id),
                        user: common.db.ObjectID(params.member._id)}, {safe: true}, function(err, result) {
                        if (err) {
        ...
        case 'send':
            validate(function(params) {
                ...
                common.db.collection('reports').findOne({_id: common.db.ObjectID(id),
                    user: common.db.ObjectID(params.member._id)}, function(err, result) {
                    if (err || !result) {
                        ...
                        reports.getReport(common.db, result, function(err, res) {
                            if (err) {
                                ...
                                if (params && params.res) {
                                    common.returnRaw(params, 200, res.message, {'Content-Type': 'text/html; charset=utf-8',
    ...
    function validateNonCoreUser(params, props, cb) {
        plugins.dispatch("/report/authorize", { params: params,
            report: props }, function() {
