                        ...
                        if (!cache[app_id] || !cache[app_id][report.period]) {
                            function metricIterator(metric, done) {
                                if (metric.indexOf("events") === 0) {
                                    ...
                                    if (metric === "crashdata") {
                                        fetch.getTimeObj(metric, params, {db: db,
                                            unique: "cru"}, function(output) {
                                            done(null, {metric: metric,
                                                data: output});
                                        });
                                    }
                                    else {
                            ...
                            if (err) {
                                ...
                                localize.getProperties(member.lang, function(err, props) {
                                    if (err) {
                                        ...
                                        var message = ejs.render(template, {"apps": report.apps,
                                            "host": host,
                                            "report": report,
                                            "version": versionInfo,
                                            "properties": props,
                                        ...
                                        if (callback) {
                                            return callback(err, {"apps": report.apps,
                                                "host": host,
                                                "report": report,
                                                "version": versionInfo,
                                                "properties": props,
