            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (result.result === 'Nothing changed') {
                        CountlyHelpers.notify({type: 'warning',
                            message: jQuery.i18n.map['management-applications.plugins.saved.nothing']});
                    }
                    else {
                        CountlyHelpers.notify({title: jQuery.i18n.map['management-applications.plugins.saved.title'],
                            message: jQuery.i18n.map['management-applications.plugins.saved']});
...
$.extend(Template.prototype, {
    ...
    fetch: function(name) {
        ...
        if (!T.isCached(name)) {
            var raw = $.ajax({ 'url': T.urlFor(name),
...
var AppRouter = Backbone.Router.extend({
    ...
    */
        ...
        submenu: {
            toggle: function(el) {
                ...
                if (el.is(":visible")) {
                    ...
                    el.css({ "right": "-110px" }).show().animate({ "right": "0" }, { duration: 300,
                        easing: 'easeOutExpo' });
                    addText();
                }
                else {
                    el.css({ "right": "-170px" }).show().animate({ "right": "0" }, { duration: 300,
                        easing: 'easeOutExpo' });
                    addText();
                }
    ...
    back: function(fallback_route) {
        if (this.routesHit > 1) {
            ...
            this.navigate(fallback_route || '/', {trigger: true,
                replace: true});
        }
    },
        ...
        */
        ...
        $(document).ready(function() {
                ...
                var resetSessionTimeouts = function(timeout) {
                    ...
                    if (minute > 0) {
                        minTimeout = setTimeout(function() {
                            CountlyHelpers.notify({ title: jQuery.i18n.map["common.session-expiration"],
                                message: jQuery.i18n.map["common.expire-minute"],
                                info: jQuery.i18n.map["common.click-to-login"] });
                        }, minute);
                    }
                    ...
                    if (tenSeconds > 0) {
                        tenSecondTimeout = setTimeout(function() {
                            CountlyHelpers.notify({ title: jQuery.i18n.map["common.session-expiration"],
                                message: jQuery.i18n.map["common.expire-seconds"],
                                info: jQuery.i18n.map["common.click-to-login"] });
                        }, tenSeconds);
                    }
                    if (logoutTimeout) {
            ...
            $("#sidebar-menu").hoverIntent({
                over: function() {
                    ...
                    if (!$(this).hasClass("menu-active") && $(".sidebar-submenu").is(":visible") && !visibleSubmenu.hasClass("half-visible")) {
                        ...
                        visibleSubmenu.animate({ "right": "-110px" }, { duration: 300,
                            easing: 'easeOutExpo' });
                    }
                },
                out: function() { },
            ...
            $("#sidebar-menu").hoverIntent({
                ...
                out: function() {
                    ...
                    if ($(".sidebar-submenu").is(":visible") && visibleSubmenu.hasClass("half-visible")) {
                        ...
                        visibleSubmenu.animate({ "right": "0" }, { duration: 300,
                            easing: 'easeOutExpo' });
                    }
                },
                selector: ""
            ...
            var help = _.once(function() {
                CountlyHelpers.alert(jQuery.i18n.map["help.help-mode-welcome"], "popStyleGreen popStyleGreenWide", {button_title: jQuery.i18n.map["common.okay"] + "!",
                    title: jQuery.i18n.map["help.help-mode-welcome-title"],
                    image: "welcome-to-help-mode"});
            });
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result === false) {
                                ...
                                updateGlobalNotes({ date_id: dateId,
                                    note: note }, "delete");
    ...
    */
    ...
    addAppManagementView: function(plugin, title, View) {
        this.appManagementViews[plugin] = {title: title,
            view: View};
    },
    /**
    ...
    */
    ...
    pageScript: function() { //scripts to be executed on each view change
        ...
        $(document).ready(function() {
            ...
            $(".usparkline").peity("bar", { width: "100%",
                height: "30",
                colour: "#83C986",
                strokeColour: "#83C986",
                strokeWidth: 2 });
            $(".dsparkline").peity("bar", { width: "100%",
                height: "30",
                colour: "#DB6E6E",
                strokeColour: "#DB6E6E",
                strokeWidth: 2 });
