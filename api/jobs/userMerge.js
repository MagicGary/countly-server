...
class UserMergeJob extends job.Job {
    run(db, done) {
        ...
        function handleMerge(app, done) {
            db.collection('app_user_merges' + app._id).find({cd: {$gte: startTime.toDate(),
                $lte: endTime.toDate()}}).toArray(function(err, res) {
                if (!err && res && res.length) {
                    ...
                    for (var i = 0; i < res.length; i++) {
                        if (res[i].merged_to) {
                            ...
                            log.d('Dispatching', {app_id: app._id + "",
                                oldUser: {uid: res[i]._id},
                                newUser: {uid: res[i].merged_to}});
                            plugins.dispatch("/i/device_id", {app_id: app._id + "",
                                oldUser: {uid: res[i]._id},
                                newUser: {uid: res[i].merged_to}});
                        }
                    }
