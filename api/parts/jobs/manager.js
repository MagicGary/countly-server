                ...
                try {
                    ...
                    let promise = toCancel && toCancel.length ? sequence(toCancel, async j => {
                            ...
                            if (job) {
                                ...
                                await JOB.Job.update(this.db, {_id: j._id}, {$set: {status: JOB.STATUS.CANCELLED,
    ...
    async process(jobs) {
        try {
            while (jobs.length) {
                ...
                let old = await JOB.Job.updateAtomically(this.db, {_id: job._id,
                    status: {$in: [STATUS.RUNNING, STATUS.SCHEDULED, STATUS.PAUSED]}}, update, false);
                if (old) {
                    if (old.status === STATUS.RUNNING) {
                        ...
                        if (!p) {
                            await JOB.Job.updateAtomically(this.db, {_id: job._json._id,
                                status: STATUS.RUNNING}, {$set: {status: STATUS.SCHEDULED}});
                        }
                        else {
                            p.catch(e => {
