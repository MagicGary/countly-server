...
(function(countlyEvents) {
    ...
    countlyEvents.processEvents = function(params) {
        return new Promise(function(resolve, reject) {
            ...
            common.db.collection("events").findOne({'_id': params.app_id}, {list: 1,
                segments: 1,
                omitted_segments: 1}, function(err, eventColl) {
                var appEvents = [],
                    appSegments = {},
                    metaToFetch = {},
                    omitted_segments = {};
    ...
    function processEvents(appEvents, appSegments, appSgValues, params, omitted_segments, done) {
        ...
        for (var i = 0; i < params.qstring.events.length; i++) {
            ...
            plugins.dispatch("/i/events", {params: params,
                currEvent: currEvent});
        ...
        if (!plugins.getConfig("api", params.app && params.app.plugins, true).safe) {
            for (var collection in eventCollections) {
                ...
                for (var segment in eventCollections[collection]) {
                    ...
                    common.db.collection(collection).update({'_id': collId}, {$set: {"m": collIdSplits[1],
                        "s": collIdSplits[0]},
                    "$inc": eventCollections[collection][segment]}, {'upsert': true}, function(err, res) {});
                }
            }
        }
        else {
            ...
            for (var collection in eventCollections) {
                ...
                for (var segment in eventCollections[collection]) {
                    ...
                    eventDocs.push({
                        "collection": collection,
                        "_id": collId,
                        "updateObj": {$set: {"m": collIdSplits[1],
                            "s": collIdSplits[0]},
            ...
            function updateEventDb(eventDoc, callback) {
                common.db.collection(eventDoc.collection).update({'_id': eventDoc._id}, eventDoc.updateObj, {'upsert': true,
