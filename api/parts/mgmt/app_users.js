...
(function(usersApi) {
                ...
                usersApi.getUid(app_id, function(err1, uid) {
                    if (uid) {
                        ...
                        common.db.collection('app_users' + app_id).insert(doc, function(err2, res) {
                            if (!err2) {
                                plugins.dispatch("/i/app_users/create", {app_id: app_id,
                                    user: doc,
                                    res: doc,
                                    params: params});
                            }
    ...
    usersApi.update = function(app_id, query, update, params, callback) {
        ...
        if (Object.keys(update).length) {
            ...
            common.db.collection('app_users' + app_id).findAndModify(query, {}, update, {upsert: true}, function(err, res) {
                if (!err) {
                    plugins.dispatch("/i/app_users/update", {app_id: app_id,
                        query: query,
                        update: update,
                        user: res && res.value,
                        params: params});
        ...
        common.db.collection("app_users" + app_id).aggregate([
            ...
            if (res && res[0] && res[0].uid && res[0].uid.length) {
                common.db.collection("metric_changes" + app_id).remove({uid: {$in: res[0].uid}}, function(err, result) {
                    plugins.dispatch("/i/app_users/delete", {app_id: app_id,
                        query: query,
                        uids: res[0].uid,
    ...
    */
    usersApi.getUid = function(app_id, callback) {
        common.db.collection('apps').findAndModify({_id: common.db.ObjectID(app_id)}, {}, {$inc: {seq: 1}}, {new: true,
    ...
    */
    usersApi.merge = function(app_id, newAppUser, new_id, old_id, new_device_id, old_device_id, callback) {
        function mergeUserData(newAppUser, oldAppUser) {
            ...
            plugins.dispatch("/i/user_merge", {app_id: app_id,
            ...
            common.db.collection('app_users' + app_id).update({_id: newAppUser._id}, {'$set': newAppUser}, function() {
                ...
                common.db.collection('app_users' + app_id).remove({_id: oldAppUser._id}, function(err, res) {
                    ...
                    plugins.dispatch("/i/device_id", {app_id: app_id,
                        oldUser: oldAppUser,
                        newUser: newAppUser});
                    if (callback) {
                        callback(err, res);
    ...
    var run_command = function(my_command) {
        return new Promise(function(resolve, reject) {
            var child = spawn(my_command, {shell: true,
                cwd: path.resolve(__dirname, './../../../export/AppUser'),
                detached: false}, function(error) {
                if (error) {
        ...
        common.db.collection("app_users" + app_id).aggregate([
            ...
            if (res && res[0] && res[0].uid && res[0].uid.length) {
                ...
                if (fs.existsSync(export_folder + '.tar.gz')) {
                    callback({message: 'There is exported data for given users on server.Delete it to start new',
                ...
                if (!fs.existsSync(export_folder)) {
                    ...
                    callback({message: 'There is ongoing export data on server with the same users.Wait till finish or delete it to start new',
                ...
                new Promise(function(resolve, reject) {
                    ...
                    function(result) {
                        ...
                        plugins.dispatch("/i/app_users/export", {app_id: app_id,
                            dbstr: dbstr,
                            export_commands: export_commands,
                            query: query,
                            uids: res[0].uid,
                            export_folder: export_folder}, function() {
                            var commands = [];
                            ...
                            Promise.all(commands).then(
                                    ...
                                    clear_out_empty_files(path.resolve(__dirname, './../../../export/AppUser/' + export_filename))//remove empty files
                                        ...
                                        .then(function() {
                                            return new Promise(function(resolve, reject) { /*save export in gridFS*/
                                                ...
                                                countlyFs.gridfs.saveFile("appUsers", my_filename, my_filename, {id: export_filename + ".tar.gz",
                                        ...
                                        .then(
                                            function(result) {
                                                fse.remove(export_folder, err => {
                                                    if (err) {
                                                        ...
                                                        callback({message: "Export successful. Export saved as given filename.  Was  unable to clean up data associated with export. You can try to delete it via api.",
                                                            ...
                                                            common.db.collection('app_users' + app_id).update({"uid": eid}, {$set: {"appUserExport": export_folder + ".tar.gz"}}, {upsert: false}, function(err, res1) {
                                                                ...
                                                                //not updated (not exist or errored)
                                                                    ...
                                                                    usersApi.deleteExport(export_filename, params, function(err, msg) {
                                                                        if (err) {
                                                                            callback({mesage: "Exporting failed. User does not exist. Unable to clean exported data",
                                                                                filename: 'appUser_' + app_id + '_' + eid}, "");
                                                                        }
                                                                        else {
                                                                            callback({mesage: "Exporting failed. User does not exist. Partially exported data deleted.",
                                                                                filename: 'appUser_' + app_id + '_' + eid}, "");
                                                                        }
                                                                    });
                                            ...
                                            function(error) {
                                                ...
                                                usersApi.deleteExport(export_filename, params, function(err, msg) {
                                                    if (err) {
                                                        callback({mesage: "Storing exported files failed. Unable to clean up file system.",
                                                            filename: 'appUser_' + app_id + '_' + eid}, "");
                                                    }
                                                    else {
                                                        callback({mesage: "Storing exported files failed. Partially exported data deleted.",
                                                            filename: 'appUser_' + app_id + '_' + eid}, "");
                                                    }
                                                });
                                            }
                                ...
                                function(error) {
                                    ...
                                    usersApi.deleteExport(export_filename, params, function(err, msg) {
                                        if (err) {
                                            callback({mesage: "Export failed. Unable to clean up file system.",
                                                filename: 'appUser_' + app_id + '_' + eid}, "");
                                        }
                                        else {
                                            callback({mesage: "Export failed. Partially exported data deleted.",
                                                filename: 'appUser_' + app_id + '_' + eid}, "");
                                        }
                                    });
                                }
