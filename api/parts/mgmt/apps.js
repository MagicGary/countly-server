...
(function(appsApi) {
    ...
    appsApi.getAllApps = function(params) {
        ...
        common.db.collection('apps').find({}).toArray(function(err, apps) {
            ...
            if (!apps || err) {
                common.returnOutput(params, {admin_of: {},
                    user_of: {}});
                return false;
            }

            var appsObj = packApps(apps);
            common.returnOutput(params, {admin_of: appsObj,
                user_of: appsObj});
            return true;
        });
    ...
    appsApi.getCurrentUserApps = function(params) {
        ...
        common.db.collection('apps').find({ _id: { '$in': adminOfAppIds } }).toArray(function(err, admin_of) {
            common.db.collection('apps').find({ _id: { '$in': userOfAppIds } }).toArray(function(err, user_of) {
                common.returnOutput(params, {admin_of: packApps(admin_of),
    ...
    appsApi.getAppsDetails = function(params) {
        ...
        common.db.collection('app_users' + params.qstring.app_id).find({}, {ls: 1,
            _id: 0}).sort({ls: -1}).limit(1).toArray(function(err, last) {
            common.db.collection('members').findOne({ _id: params.app.owner }, {full_name: 1,
                username: 1}, function(err, owner) {
                if (owner) {
                ...
                common.db.collection('members').find({ global_admin: true }, {full_name: 1,
                    username: 1}).toArray(function(err, global_admins) {
                    common.db.collection('members').find({ admin_of: params.qstring.app_id }, {full_name: 1,
                        username: 1}).toArray(function(err, admins) {
                        common.db.collection('members').find({ user_of: params.qstring.app_id }, {full_name: 1,
    ...
    appsApi.createApp = function(params) {
        ...
        common.db.collection('apps').insert(newApp, function(err, app) {
            ...
            common.db.collection('app_users' + app.ops[0]._id).ensureIndex({"lac": 1,
                "ls": 1}, { background: true }, function(err, res) {});
            ...
            common.db.collection('app_user_merges' + app.ops[0]._id).ensureIndex({cd: 1}, {expireAfterSeconds: 60 * 60 * 3,
            ...
            plugins.dispatch("/i/apps/create", {params: params,
                appId: app.ops[0]._id,
                data: newApp});
            ...
            if (err || !appBefore) {
                ...
                if (params.member && params.member.global_admin) {
                    common.db.collection('apps').update({'_id': common.db.ObjectID(params.qstring.args.app_id)}, {$set: updatedApp}, function(err, app) {
                        plugins.dispatch("/i/apps/update", {params: params,
            ...
            Object.keys(params.qstring.args).forEach(k => {
                if (plugins.getPlugins().indexOf(k) !== -1) {
                    promises.push(new Promise((resolve, reject) => {
                        plugins.dispatch('/i/apps/update/plugins/' + k, {params: params,
                            app: app,
                            config: params.qstring.args[k]}, (err, changes) => {
                            if (err) {
                                reject(err);
                            }
                            else if (changes) {
                                ...
                                common.dbPromise('apps', 'updateOne', {_id: app._id}, {$set: {[`plugins.${k}`]: params.qstring.args[k]}}).then(() => {
                                    plugins.dispatch('/systemlogs', {params: params,
                                        ...
                                        data: {before: common.dot(app, `plugins.${k}` || {}),
    ...
    appsApi.deleteApp = function(params) {
        ...
        function removeApp(app) {
            common.db.collection('apps').remove({'_id': common.db.ObjectID(appId)}, {safe: true}, function(err, result) {
                ...
                common.db.collection('members').update({}, {$pull: {'apps': appId,
                    'admin_of': appId,
                    'user_of': appId}}, {multi: true}, function(err, app) {});
        ...
        common.db.collection('apps').findOne({'_id': common.db.ObjectID(appId)}, function(err, app) {
            if (!err && app) {
                if (params.member.global_admin) {
                    ...
                    common.db.collection('members').findOne({ admin_of: appId,
                        api_key: params.member.api_key}, function(err, member) {
                        if (!err && member) {
                            deleteAppData(appId, false, params, app);
                            common.returnMessage(params, 200, 'Success');
                        }
        ...
        common.db.collection('app_users' + appId).drop(function() {
            if (!fromAppDelete) {
                ...
                common.db.collection('app_user_merges' + appId).drop(function() {
                    common.db.collection('app_user_merges' + appId).ensureIndex({cd: 1}, {expireAfterSeconds: 60 * 60 * 3,
                        background: true}, function(err, res) {});
                });
                if (params.qstring.args.period === "reset") {
                    plugins.dispatch("/i/apps/reset", {params: params,
                        appId: appId,
                        data: app}, deleteEvents);
                }
                else {
                    plugins.dispatch("/i/apps/clear_all", {params: params,
                        appId: appId,
                        data: app}, deleteEvents);
                }
            }
                ...
                common.db.collection('metric_changes' + appId).drop(function() {});
                common.db.collection('app_user_merges' + appId).drop(function() {});
                plugins.dispatch("/i/apps/delete", {params: params,
                    appId: appId,
                    data: app}, deleteEvents);
            }
        });
    ...
    function deletePeriodAppData(appId, fromAppDelete, params, app) {
        ...
        plugins.dispatch("/i/apps/clear", {params: params,
            appId: appId,
            data: app,
            moment: now,
            dates: dates,
