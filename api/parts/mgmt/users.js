...
(function(usersApi) {
    ...
    usersApi.getUserById = function(params) {
        ...
        common.db.collection('members').findOne({ _id: common.db.ObjectID(params.qstring.id) }, { password: 0,
            appSortList: 0 }, function(err, member) {
    ...
    usersApi.getAllUsers = function(params) {
        ...
        common.db.collection('members').find({}, { password: 0,
            appSortList: 0 }).toArray(function(err, members) {
    ...
    usersApi.createUser = function(params) {
        ...
        var argProps = {
                ...
                'lang': { 'required': false,
                    'type': 'String' },
                'admin_of': { 'required': false,
                    'type': 'Array' },
                'user_of': { 'required': false,
                    'type': 'Array' },
                'global_admin': { 'required': false,
                    'type': 'Boolean' }
            },
            newMember = {};
        ...
        function createUser() {
            ...
            common.db.collection('members').insert(newMember, { safe: true }, function(err, member) {
                ...
                if (member && member.length && !err) {
                    ...
                    plugins.dispatch("/i/users/create", { params: params,
                        data: member[0] });
    ...
    usersApi.updateUser = function(params) {
        ...
        function killAllSessionForUser(userId) {
            ...
            //delete auth tokens
            common.db.collection('auth_tokens').remove({ 'owner': userId,
            ...
            common.db.collection('members').update({ '_id': common.db.ObjectID(params.qstring.args.user_id) }, { '$set': updatedMember }, { safe: true }, function(err, isOk) {
                common.db.collection('members').findOne({ '_id': common.db.ObjectID(params.qstring.args.user_id) }, function(err, member) {
                    if (member && !err) {
                        ...
                        plugins.dispatch("/i/users/update", { params: params,
                            data: updatedMember,
                            member: memberBefore });
        ...
        for (var i = 0; i < userIds.length; i++) {
            ...
            if (!userIds[i] || userIds[i] === params.member._id + "" || userIds[i].length !== 24) {
                ...
                common.db.collection('members').findAndModify({ '_id': common.db.ObjectID(userIds[i]) }, {}, {}, { remove: true }, function(err, user) {
                    if (!err && user && user.ok) {
                        plugins.dispatch("/i/users/delete", { params: params,
                            data: user.value });
                    }
                });
            }
