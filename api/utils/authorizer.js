...
(function(authorizer) {
        ...
        if (options.owner && options.owner !== "") {
            ...
            options.db.collection('members').findOne({'_id': options.db.ObjectID(options.owner)}, function(err, member) {
                if (err) {
                    ...
                    options.db.collection("auth_tokens").insert({_id: options.token,
                        ttl: options.ttl,
                        ...
                        purpose: options.purpose}, function(err, res) {
                        if (typeof options.callback === "function") {
    ...
    */
    authorizer.clean = function(options) {
        options.db = options.db || common.db;
        options.db.collection("auth_tokens").remove({ends: {$lt: Math.round(Date.now() / 1000)},
            ttl: {$ne: 0}}, options.callback);
    };
