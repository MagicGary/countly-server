...
(function(taskmanager) {
    ...
    */
    taskmanager.checkResult = function(options, callback) {
        options.db = options.db || common.db;
        options.db.collection("long_tasks").findOne({_id: options.id}, {_id: 0,
            status: 1}, callback);
    };
    ...
    */
    taskmanager.rerunTask = function(options, callback) {
        ...
        function runTask(options, reqData, callback) {
            options.db.collection("long_tasks").update({_id: options.id}, {$set: {status: "rerunning",
                start: new Date().getTime()}}, function(err, res) {
