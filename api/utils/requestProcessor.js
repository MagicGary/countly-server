                        ...
                        taskmanager.checkIfRunning({
                            ...
                            if (task_id) {
                                ...
                                countlyApi.mgmt.appUsers.export(params.qstring.app_id, params.qstring.query || {}, params, taskmanager.longtask({
                                    ...
                                    meta: JSON.stringify({"app_id": params.qstring.app_id,
            ...
            case '/i/tasks': {
                ...
                case 'update':
                    validateUserForWrite(params, () => {
                        taskmanager.rerunTask({db: common.db,
                            id: params.qstring.task_id}, (err, res) => {
                ...
                case 'delete':
                    validateUserForWrite(params, () => {
                        taskmanager.deleteResult({db: common.db,
                            id: params.qstring.task_id}, (err, res) => {
                            common.returnMessage(params, 200, "Success");
                        });
                    });
                            ...
                            common.db.collection('events').update({"_id": common.db.ObjectID(app_id)}, updateThese, function(err, events) {
                                if (err) {
                                    ...
                                    for (var i = 0; i < idss.length; i++) {
                                        ...
                                        plugins.dispatch("/i/event/delete", {event_key: idss[i],
                                            appId: app_id});
                                    }
                                    plugins.dispatch("/systemlogs", {params: params,
            ...
            case '/o/tasks': {
                ...
                case 'all':
                    validateUserForMgmtReadAPI(() => {
                        ...
                        taskmanager.getResults({db: common.db,
                            query: params.qstring.query}, (err, res) => {
                ...
                case 'task':
                    validateUserForMgmtReadAPI(() => {
                        ...
                        taskmanager.getResult({db: common.db,
                            id: params.qstring.task_id}, (err, res) => {
                            if (res) {
                                common.returnOutput(params, res);
                            }
                ...
                case 'check':
                    validateUserForMgmtReadAPI(() => {
                        ...
                        taskmanager.checkResult({db: common.db,
                            id: params.qstring.task_id}, (err, res) => {
                            if (res) {
                ...
                case 'delete':
                    validateUser(() => {
                        if (params.qstring.tokenid) {
                            common.db.collection("auth_tokens").remove({"_id": params.qstring.tokenid,
                                "owner": params.member._id + ""}, function(err, res) {
                                if (err) {
            ...
            case '/o/token': { //returns all my tokens
                ...
                case 'check':
                    ...
                    validateUser(params, function() {
                        authorize.check_if_expired({token: params.qstring.token,
                            db: common.db,
                            callback: (err, valid, time_left)=>{
                                if (err) {
                                    ...
                                    common.returnMessage(params, 200, {valid: valid,
                                        time: time_left});
                                }
                            }
                        });
...
const processRequestData = (params, app, done) => {
    plugins.dispatch("/i", {params: params,
        app: app}, () => {
        if (params.qstring.events) {
            if (params.promises) {
...
const validateAppForWriteAPI = (params, done, try_times) => {
    ...
    common.db.collection('apps').findOne({'key': params.qstring.app_key}, (err, app) => {
        ...
        common.db.collection('app_users' + params.app_id).findOne({'_id': params.app_user_id}, (err, user) => {
            ...
            plugins.dispatch("/sdk", {params: params,
                app: app}, () => {
                        ...
                        countlyApi.mgmt.appUsers.getUid(params.app_id, function(err, uid) {
                            if (uid) {
                                ...
                                if (!params.app_user._id) {
                                    ...
                                    common.db.collection('app_users' + params.app_id).insert({_id: params.app_user_id,
                                        uid: uid,
                                        did: params.qstring.device_id}, function(err, res) {
                                    ...
                                    common.db.collection('app_users' + params.app_id).update({_id: params.app_user_id,
                                        uid: {$exists: false}}, {$set: {uid: uid}}, {upsert: true}, function(err, res) {
                                        restartRequest(params, done, try_times);
                                    });
                                }
                            }
