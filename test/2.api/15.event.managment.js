...
function generate_data(z) {
    it('Adding sgmented data(to get big list if possible) ' + (z + 1), function(done) {
        ...
        for (var p = 0; p <= 21; p++) {
            var newobj = {"key": "t1",
                "count": 5,
                "timeStamp": (p + z * 21) * 1000,
                "segmentation": {"s": "v_" + z + "_" + p}};
            params.push(newobj);
        }
...
describe('Testing event settings', function() {
    ...
    describe('setting map values', function() {
        it('setting map', function(done) {
            var event_map = {"test2": {"name": "My Test name",
                "desc": "My desc"}};
            request
        ...
        it('validating result', function(done) {
            request
                ...
                .end(function(err, res) {
                    ...
                    var ob = JSON.parse(res.text);
                    ob.map.should.have.property("test2", {"name": "My Test name",
                        "desc": "My desc"});
                    done();
                });
        });
        ...
        it('validating result', function(done) {
            request
                ...
                .end(function(err, res) {
                    ...
                    var ob = JSON.parse(res.text);
                    ob.should.have.property("overview", [{"eventKey": "test2",
                        "eventProperty": "count"}]);
                    done();
                });
        });
    ...
    describe('editing overview(with duplicate)', function() {
        it('setting order', function(done) {
            var overview = [{"eventKey": "test2",
                "eventProperty": "count"}, {"eventKey": "test1",
                "eventProperty": "count"}, {"eventKey": "test1",
                "eventProperty": "count"}];
            request
                .get('/i/events/edit_map?app_id=' + APP_ID + '&api_key=' + API_KEY_ADMIN + "&event_overview=" + JSON.stringify(overview))
    ...
    describe('creating events with segments', function() {
        it('create test events', function(done) {
            var params = [ {"key": "test3",
                "count": 1,
                "sum": 5,
                "dur": 10,
                "segmentation": {"my_segment": "value",
                    "my_segment2": "value"}}];
            ...
            setTimeout(function() {
                request
                    ...
                    .end(function(err, res) {
                        ...
                        ob.should.have.property("order", ["test1"]);
                        ob.should.have.property("overview", []);
                        ob.should.have.property("segments", {"test3": ["my_segment", "my_segment2"],
                            "t1": ["s"]});
                        done();
                    });
            }, 5000);
            ...
            setTimeout(function() {
                request
                    ...
                    .end(function(err, res) {
                        ...
                        ob.should.have.property("order", ["test1"]);
                        ob.should.have.property("overview", []);
                        ob.should.have.property("segments", {"test3": ["my_segment2"],
                            "t1": ["s"]});
